//校团委web页面js文件
$(function () {
	// http://101.201.146.103:5003/api/activities?university=1
	// http://101.201.146.103:5003/api/communities/list?university=1
	//获取学校id
	...
	var load_act_list = function (curr_page) {
		$.ajax({
			type: 'GET',
			url: '/api/activities?university='+un_id+'&page='+curr_page,
			dataType: 'json',
	...
	var load_org_list = function (curr_page) {
		$.ajax({
			type: 'GET',
			url: '/api/communities/list?university='+un_id+'&page='+curr_page,
			dataType: 'json',
			success: function (data) {
				var items = data.data.items;
				//组织总数
