angular.module('iwx')
	.controller('infoConllectCtrl', function ($scope, $http, $rootScope, $state, eventType) {
		//确认框信息
		$scope.confirm = {};
		...
		$http.get('/api/admin/community/register/form')
			...
			$scope.$parent.$watch('community.enable_register', function(newVal, oldVal) {
				if (newVal !== undefined && oldVal !== undefined && newVal !== oldVal) {
					if (newVal) {
						$http.post('/api/admin/community/register')
							.success(function() {
								$rootScope.$emit(eventType.NOTIFICATION, {
									'type': 'POPMSG',
									'title': '消息',
						...
						$http.delete('/api/admin/community/register')
							.success(function() {
								$rootScope.$emit(eventType.NOTIFICATION, {
									'type': 'POPMSG',
									'title': '消息',
